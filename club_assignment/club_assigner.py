...
class ClubAssigner:
    ...
    def clustering_bak(self, img: np.ndarray, k: int = 2) -> Tuple[int, int, int]:
        """
        Cluster the dominant color from a masked image using HSV k-means clustering.

        Args:
            img (np.ndarray): Masked BGR image.
            k (int): Number of clusters.

        Returns:
            Tuple[int, int, int]: Dominant jersey color in BGR format.
        """
        # Remove black background pixels
        pixels = img.reshape(-1, 3)
        pixels = pixels[np.any(pixels > 30, axis=1)]  # Ignore dark pixels (noise, mask)
        ...
        return tuple(int(x) for x in dominant_bgr)
    ...
    def assign_clubs(self, frame: np.ndarray, tracks: Dict[str, Dict[int, Any]]) -> Dict[str, Dict[int, Any]]:
        ...
        for track_type in ['goalkeeper', 'player']:
            for player_id, track in tracks[track_type].items():
                ...
                tracks[track_type][player_id]['type'] = is_goalkeeper
