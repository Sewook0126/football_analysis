...
class FootballVideoProcessor(AbstractAnnotator, AbstractVideoProcessor):
    ...
    def __init__(self, obj_tracker: ObjectTracker, kp_tracker: KeypointsTracker, 
        ...
        Args:
            ...
            scale_x (float, optional): Force X-axis scale factor (meters per pixel). If None, auto-calculate.
            scale_y (float, optional): Force Y-axis scale factor (meters per pixel). If None, auto-calculate.
        """
        ...
        self.speed_estimator = SpeedEstimator(
            field_image.shape[1], 
            field_image.shape[0], 
            verbose_logging=True,  # 启用详细的速度日志
            scale_x=scale_x,       # 使用传入的X轴比例（如果有）
            scale_y=scale_y        # 使用传入的Y轴比例（如果有）
        )
    ...
    def process(self, frames: List[np.ndarray], fps: float = 1e-6) -> List[np.ndarray]:
        ...
        for idx, (frame, object_detection, kp_detection) in enumerate(zip(frames, batch_obj_detections, batch_kp_detections)):
            ...
            # 调试信息：显示关键点数量
            if self.frame_num % 30 == 0:  # 每30帧输出一次
                print(f"[Frame {self.frame_num}] 关键点数量: {len(kp_tracks)}")
                if len(kp_tracks) > 0:
                    # 显示前3个关键点的位置
                    sample_kps = list(kp_tracks.items())[:3]
                    for kp_id, (x, y) in sample_kps:
            ...
            all_tracks['object'] = self.speed_estimator.calculate_speed(
                all_tracks['object'], self.frame_num, self.cur_fps, all_tracks['keypoints']
            )

            all_tracks['object'] = self.speed_estimator.calculate_opponent_distances(all_tracks['object'])
    ...
    def annotate(self, frame: np.ndarray, tracks: Dict) -> np.ndarray:
        ...
        combined_frame = self._annotate_possession(frame)
    ...
    def _combine_frame_projection(self, frame: np.ndarray, projection_frame: np.ndarray) -> np.ndarray:
        ...
        # Use the frame dimensions as canvas size (dynamic sizing)
        ...
        # Create a blank canvas with dynamic size
