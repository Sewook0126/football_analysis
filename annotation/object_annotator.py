...
class ObjectAnnotator(AbstractAnnotator):
        ...
        for track in tracks:
            for track_id, item in tracks[track].items():
                ...
                if track == 'ball':
                    ...
                    opponent_distance = item.get('nearest_opponent_distance', None)
                    frame = self.draw_ellipse(frame, item['bbox'], color, track_id, speed, track, opponent_distance)
    ...
    def draw_ellipse_bak(self, frame: np.ndarray, bbox: Tuple[int, int, int, int], color: Tuple[int, int, int],
    ...
    def draw_ellipse(self, frame: np.ndarray, bbox: Tuple[int, int, int, int], color: Tuple[int, int, int],
                     track_id: int, speed: float, obj_cls: str = 'player',
                     opponent_distance: float = None) -> np.ndarray:
        """
        Draws an ellipse around an object and annotates it with its ID, speed, and optionally the distance to the nearest opponent.
        ...
        Args:
            ...
            speed (float): The speed of the object (in km/h).
            obj_cls (str): The object class, either 'player', 'goalkeeper', or 'referee'.
            opponent_distance (float, optional): The distance to the nearest opponent (in meters).
        ...
        # Draw different ellipse styles
        if obj_cls == 'referee':
            self._draw_dashed_ellipse(frame, x, y, w, color)
        elif obj_cls == 'goalkeeper':
            self._draw_double_ellipse(frame, x, y, w, color)
        else:
            cv2.ellipse(frame, center=(x, y), axes=(w, 20), angle=0, startAngle=-30, endAngle=240,
                        color=color, thickness=2, lineType=cv2.LINE_AA)

        # Draw ID background rectangle
        y = int(bbox[3]) + 10
        h, w = 10, 20
        cv2.rectangle(frame, (x - w, y - h), (x + w, y + h), color, cv2.FILLED)
        ...
        # ID text
        x1 = x - len(str(track_id)) * 5
        cv2.putText(frame, text=f"{track_id}", org=(x1, y + h // 2), fontFace=cv2.FONT_HERSHEY_PLAIN,
                    fontScale=1, color=color2, thickness=2)

        # Speed text
        if speed >= 0:
            speed_str = f"{speed:.2f} km/h"
            x2 = x - len(speed_str) * 5
            cv2.putText(frame, text=speed_str, org=(x2, y + 20), fontFace=cv2.FONT_HERSHEY_PLAIN,
                        fontScale=1, color=color2, thickness=2)

        # Distance to opponent (drawn below speed)
        if opponent_distance is not None and opponent_distance > 0:
            distance_str = f"{opponent_distance:.2f} m"
            x3 = x - len(distance_str) * 5
            cv2.putText(frame, text=distance_str, org=(x3, y + 35), fontFace=cv2.FONT_HERSHEY_PLAIN,
                        fontScale=1, color=color2, thickness=2)
