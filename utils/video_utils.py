...
def process_video(processor = None, video_source: str = 0, output_video: Optional[str] = "output.mp4", 
                  batch_size: int = 30, skip_seconds: int = 0, target_resolution: tuple = None) -> None:
    """
    ...
    Args:
        ...
        target_resolution (tuple, optional): Target resolution for frame resizing (width, height). 
    ...
    if target_resolution:
        ...
        print(f"📐 视频尺寸调整: {original_width}x{original_height} → {target_width}x{target_height}")
    else:
        target_width, target_height = original_width, original_height
        print(f"📐 使用原始视频尺寸: {original_width}x{original_height}")
    ...
    def frame_display_thread(temp_dir: str) -> None:
        ...
        # 设置窗口属性 - 允许调整大小
        cv2.namedWindow('Football Analysis', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
        # 设置窗口初始大小 (宽度, 高度) - 可以根据需要调整这些值
        cv2.resizeWindow('Football Analysis', 960, 540)  # 设置为较小的窗口大小
    ...
    try:
        ...
        if hasattr(processor, 'speed_estimator'):
            ...
            print(f"Speed analysis saved to: {speed_file}")
